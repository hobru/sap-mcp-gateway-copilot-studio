...
This is the follow-up to *"MCP Gateway on BTP Integration Suite"* ([guide](./01-integration-suite.md) · [video](https://youtu.be/1m12OVONavA)). The MCP server setup on SAP Integration Suite is the same — same Star Wars API, same exposed tools. What changes here is **how we authenticate**:
...
2. **Edit → Code** view: enable the code and replace the script with [`custom-connector-script.csx`](../scripts/custom-connector-script.csx), which sets the outgoing `Content-Type` to exactly `application/json`.
...
To see exactly what Copilot Studio does behind the scenes, reproduce the flow with the REST Client using [`entra-id-auth.http`](../http/entra-id-auth.http):
...
> **Follow-up guide:** [*MCP Gateway with SAP IAS (federated to Entra ID)*](./03-sap-ias.md) puts **SAP Cloud Identity Services (IAS)** in front as the token issuer, with Entra ID federated into IAS. Because the gateway then validates an **IAS-issued** token, it sets up **principal propagation to an on-prem ABAP backend** as a purely additive next step.

---

## VS Code — Entra ID OAuth + MCP calls

Full snippet: [`entra-id-auth.http`](../http/entra-id-auth.http). Highlights:
...
The custom connector that Copilot Studio auto-creates runs this **C# script** so the outgoing request carries an exact `Content-Type: application/json`. See [`custom-connector-script.csx`](../scripts/custom-connector-script.csx).
