...
A lot of customers want to use **Microsoft Copilot Studio** against their SAP **Sales**, **Finance**, and **Procurement** scenarios. To make that concrete, this repo ships **three ready-made SAP Integration Suite content packages** — one per area — that you can import as a starting point. Each package fronts standard **OData services** from an **S/4HANA** system — the same services documented in the [SAP Business Accelerator Hub](https://api.sap.com/) — and exposes them as **MCP servers** through the SAP **MCP Gateway**.

To consume those MCP servers in Copilot Studio, a **Power Platform custom connector** has to be created for each one, and each connector then surfaces as an **MCP tool** the agent can call. Doing that by hand means running the connector wizard once per endpoint, re-typing the same **SAP IAS** OAuth values, and re-applying a C# `Content-Type` fix every single time. Across the **21 endpoints** in the three packages — **Sales (7), Finance (6), Procurement (8)** — that is 21 chances to fumble a redirect, a scope, or a script paste. Instead of doing this manually, this guide automates it: you scaffold the connector definitions once and deploy them with `pac connector create`, which sets the IAS OAuth configuration **and** embeds the custom C# script in a **single call**.
