...
This is the follow-up to *"MCP Gateway — user authentication with Microsoft Entra ID"* ([guide](./02-entra-id.md) · [video](https://www.youtube.com/watch?v=jE-qlg2vZ6I)), which in turn built on *"MCP Gateway on BTP Integration Suite"* ([guide](./01-integration-suite.md) · [video](https://youtu.be/1m12OVONavA)). The MCP server setup on SAP Integration Suite is the same — same Star Wars API, same exposed tools. What changes here is **who issues the token**:
...
![Phase 1 architecture: Copilot Studio → IAS (federated to Entra ID) → MCP Gateway → SWAPI, with Phase 2 adding principal propagation to an on-prem ABAP backend](../assets/architecture-ias-mcp.svg)
...
- A deployed **MCP Server** artifact on Integration Suite exposing the Star Wars API (see the [first guide](./01-integration-suite.md) for the build steps and [`swapi-openapi-301.yaml`](../openapi/swapi-openapi-301.yaml)).
...
**✅ Verify 1 — endpoints & issuer sanity.** Run [`verify-step1-discovery.http`](../http/verify-step1-discovery.http). Expected:
...
**✅ Verify 2 — full auth-code flow (proves Step 1 *and* Step 2).** Use [`verify-step2-ias-token.http`](../http/verify-step2-ias-token.http).
...
2. **Edit → Code** view: enable the code and replace the script with [`custom-connector-script.csx`](../scripts/custom-connector-script.csx), which forces the outgoing `Content-Type` to exactly `application/json` and leaves the **Authorization** (IAS bearer) and **Accept** headers untouched.
...
- [`verify-step1-discovery.http`](../http/verify-step1-discovery.http) — confirms the IAS + Entra discovery documents resolve and the **IAS issuer** is correct (what the gateway will trust).
- [`verify-step2-ias-token.http`](../http/verify-step2-ias-token.http) — the full IAS authorization-code flow (Microsoft login → IAS token), decode the JWT, then optionally call the MCP Gateway.
...
- Previous guide — MCP Gateway with Microsoft Entra ID — [`02-entra-id.md`](./02-entra-id.md)
- First guide — MCP Gateway on SAP Integration Suite — [`01-integration-suite.md`](./01-integration-suite.md)
